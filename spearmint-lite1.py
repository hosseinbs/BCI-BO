...
def main_controller(options, args):
    ...
    for i in xrange(0,options.num_jobs):
        ...
        #set value of number of the features in each tree node 
        # based on the window size 
#        candidates_list = delete_false_candidates(candidates, gmap)
#        candidates = np.array(candidates_list)
#        candidates = remove_complete_pending_jobs(candidates, pending, complete)
...
def delete_false_candidates(candidates, gmap):
    
    trial_size = 400   # TODO: fix this
    n_channels = 59 # TODO: fix this
    indices_dic = {}
    
    for variable_ind , variable in enumerate(gmap.variables):
        
        if variable['name'] == 'windowSize':
            indices_dic['windowSize'] = variable_ind
        
        elif variable['name'] == 'discarded_samples_begining_nc':
            indices_dic['discarded_samples_begining_nc'] = variable_ind
        
        elif variable['name'] == 'discarded_samples_end_nc':
            indices_dic['discarded_samples_end_nc'] = variable_ind
        
        elif variable['name'] == 'discarded_samples_begining_movement':
            indices_dic['discarded_samples_begining_movement'] = variable_ind
        
        elif variable['name'] == 'discarded_samples_end_movement': 
            indices_dic['discarded_samples_end_movement'] = variable_ind
        
        elif variable['name'] == 'max_features': 
            indices_dic['max_features'] = variable_ind
        
        elif variable['name'] == 'max_depth': 
            indices_dic['max_depth'] = variable_ind
    ...
    for candidate in candidates:
        ...
        u = gmap.unit_to_list(candidate)
        
        rem_mv_samples = trial_size - u[indices_dic['discarded_samples_end_movement']] - u[indices_dic['discarded_samples_begining_movement']]   
        rem_nc_samples = trial_size - u[indices_dic['discarded_samples_begining_nc']] - u[indices_dic['discarded_samples_end_nc']]
    
        wind_size = int(u[indices_dic['windowSize']])
        if not (rem_mv_samples < wind_size or  rem_nc_samples < wind_size 
                or wind_size * n_channels <  u[indices_dic['max_features']] or 
                wind_size * n_channels <  u[indices_dic['max_depth']]):
            temp.append(candidate.tolist())
...
def remove_complete_pending_jobs(candidates, pending, complete):
        
    for p in pending:
        other_rows = (candidates != p).any(axis=1)  # Rows that have at least one element that differs
        other_rows = np.array(other_rows).squeeze()
        candidates = candidates[other_rows,:]
        
    for c in complete:
        other_rows = (candidates != c).any(axis=1)  # Rows that have at least one element that differs
        other_rows = np.array(other_rows).squeeze()
        candidates = candidates[other_rows,:]
