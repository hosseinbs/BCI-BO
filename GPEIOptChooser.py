...
def init(expt_dir, arg_string, subj):#, gmap, spearmint):
    args = util.unpack_args(arg_string)
    return GPEIOptChooser(expt_dir, subj, **args) #gmap, spearmint,**args)
"""
...
class GPEIOptChooser:
    ...
    def __init__(self, expt_dir, subject_info, covar="Matern52", mcmc_iters=10, 
                 pending_samples=100, noiseless=False, burnin=100,
                 grid_subset=20):
#         gmap, spearmint ,covar="Matern52", mcmc_iters=10, 
#                  pending_samples=100, noiseless=False, burnin=100,
#                  grid_subset=20):
        
        self.subject = subject_info
#         self.spearmint = spearmint
#         self.gmap = gmap
        self.cov_func        = getattr(gp, covar)
        self.locker          = Locker()
        self.state_pkl       = os.path.join(expt_dir, self.__module__ + subject_info + ".pkl")
        self.stats_file      = os.path.join(expt_dir, 
                                   self.__module__ + "_hyperparameters" + subject_info +".txt")
...
#         new_points = np.random.randn(50,comp.shape[1])*0.001 + comp[best_comp,:]
#         new_points = self.spearmint.delete_false_candidates(new_points, self.gmap, 0, self.subject)
#         new_points = self.spearmint.remove_complete_pending_jobs(new_points, pend, comp, self.gmap)
#         cand2 = np.vstack((new_points, cand))
#         cand2 = np.copy(cand)
...
#                 cand2[i,:] = ret[0]
# #
# #             cand2 = self.spearmint.delete_false_candidates(cand2, self.gmap, 0, self.subject)
# #             
# #             if len(cand2) != 0:
# #                 cand2 = self.spearmint.remove_complete_pending_jobs(cand2, pend, comp, self.gmap)
# #            cand2 = []
#             if len(cand2) != 0:
