...
class Data_Preprocessor:
    ...
    def __init__(self, config, subject, feature_extractor_name, number_of_CSPs):#, classifier_name):
        """"""
...
#         if 'dynamic' in classifier_name:
#             self.classifier_type = 'dynamic'
#             self.set_dynimic_feature_extractor(self.classifier_name)
#         else:
...
#             self.feature_extractor = FE.Feature_Extractor.bp_feature_extractor
    ...
    def set_dynimic_feature_extractor(self, classifier_name):
        """This function is only called if the classifier is dynamic"""

# check error ->                self.feature_extractor_name = feature_extractor_name

        self.features_path = os.path.join( self.features_path, classifier_name)
#         if classifier_name == 'DHMM_dynamic':
#             self.dynamic_feature_extractor = FE.Dynamic_Feature_extractor.DHMM_feature_extractor
#         elif classifier_name == "HMSVM_dynamic":
#             self.dynamic_feature_extractor = FE.Dynamic_Feature_extractor.HMSVM_feature_extractor
...
#        print(os.getcwd())
    ...
    def extract_data_samples(self, subject, params_dict, optimal, fe_param, cutoff_frequencies_low_list = None, cutoff_frequencies_high_list = None):
        ...
        if optimal:
            ...
            raw_X_train, raw_Y_train = self.load_dataset_train(subject)
            ...
            raw_X_train, raw_Y_train = np.copy(raw_X), np.copy(raw_Y)
        ...
        if cutoff_frequencies_low_list is None and self.feature_extractor_name != 'morlet':
            ...
            cutoff_frequencies_low = np.tile(np.array(cutoff_frequencies_low_list), (raw_X.shape[1],1))
            cutoff_frequencies_high = np.tile(np.array(cutoff_frequencies_high_list), (raw_X.shape[1],1))
        ...
        for filt_number in range(cutoff_frequencies_low.shape[1]):
            ...
            filtered_X_train = np.copy(raw_X_train) ##copying the list- always copy lists
            ...
            for i in range(raw_X.shape[1]):
                ...
                if optimal:
                    filtered_X_train[:,i] = self.apply_filter(filtered_X_train[:,i], cutoff_freq_low[i], cutoff_freq_high[i], self.config.configuration["sampling_rate"])
            
            if self.number_of_CSPs != -1:

                if optimal:
                    csps = self.apply_CSP(filtered_X_train, raw_Y_train)
                else:
                    csps = self.apply_CSP(filtered_X, raw_Y_train)

                filtered_X = np.dot(filtered_X, csps)

                self.logging.info('After applying CSP filters X size is: %s', str(filtered_X.shape))

            if self.config.configuration["dataset_type_str"] == 'async' and self.classifier_type == 'dynamic':

                self.logging.info('started extracting features from the data for async BCI with dynamic classifiers!')
                X_new, Y_new = self.prepare_samples_async_dynamic(subject_number, filtered_X, raw_Y, params_dict, fe_param)

            elif self.config.configuration["dataset_type_str"] == 'async' and self.classifier_type == 'static':
            ...
            if filt_number == 0:
                ...
                if self.config.configuration["dataset_type_str"] == 'async' and self.classifier_type == 'dynamic':
                    X = np.concatenate((X, np.array(X_new)), axis=2)
                else:
    ...
    def prepare_samples_async_opt(self, subject_number, X_test_raw, Y_test_raw, params_dict, fe_params):    
        ...
        window_not_overlap_size = 1
        
        x_shape = [0,0]
        x_shape[0] = len(range(0, len(X_test_raw) - window_size + 1, window_not_overlap_size))
        x_shape[1] = X_test_raw.shape[1] * window_size
        X_test = np.ndarray(x_shape, dtype=float)
        Y_test = np.ndarray(x_shape[0])
        
        X_test, num_of_added_samples = self.feature_extractor( X_test_raw, window_size, window_not_overlap_size, fe_params)
    ...
    def prepare_samples_async(self, subject_number, raw_X, raw_Y, params_dict, fe_params):    
        ...
        X_new = []
        Y_new = []    
        discard_mv_begin = params_dict['discard_mv_begin']
        discard_mv_end = params_dict['discard_mv_end']
        discard_nc_begin = params_dict['discard_nc_begin']
        discard_nc_end = params_dict['discard_nc_end']
        window_size = int(params_dict['window_size'])
        window_not_overlap_size = window_size - int(params_dict['window_overlap_size'])
        ...
        for mv_trail_begin_ind, mv_trial_begin in enumerate(mv_trials_begin):
            ...
            X_new, Y_new = self.extract_feature_each_window( mv_sample, X_new, Y_new, window_size, window_not_overlap_size, raw_Y[mv_trial_begin + 1], fe_params)
            ...
            X_new, Y_new = self.extract_feature_each_window( nc_sample, X_new, Y_new, window_size, window_not_overlap_size, raw_Y[nc_trial_begin + 1], fe_params)
        
        return X_new, Y_new


    def prepare_samples_async_dynamic(self, subject_number, raw_X, raw_Y, params_dict, fe_params):
        """In asynchronous datasets, I assumed that each trial begins with the movement start so I was able to concatenate the  NC parts
        of two consecutive trials. This way feature extraction was easier."""

        window_size = int(params_dict['window_size'])
        window_not_overlap_size = window_size
        chain_length = int(params_dict['chain_length'])

        x_shape = [0,0]
        x_shape[0] = len(range(0, (len(raw_X) - window_size + 1)))
        x_shape[1] = raw_X.shape[1] * window_size
        # X_test = np.ndarray(x_shape, dtype=float)
        # Y_test = np.ndarray(x_shape[0])

        X_each_window_list, num_of_added_samples = self.feature_extractor(raw_X, window_size, window_not_overlap_size, fe_params)
        first_indices_of_data_sample = range(0, int(raw_X.shape[0] - window_size + 1), int(window_not_overlap_size))
        Y = raw_Y[first_indices_of_data_sample]

        X_new, Y_new = self.dynamic_feature_extractor(X_each_window_list, Y, chain_length)

        self.logging.info('Done extracting features from test data! X_new.shape is %s and Y_new.shape is: %s', str(len(X_new)), str(len(Y_new)))
        return X_new, Y_new
...
#         params_list = map(int,params_list)
        X_new = []
        Y_new = []    
        discard_mv_begin = int(params_dict['discard_mv_begin'])#params_list[0]
        discard_mv_end = int(params_dict['discard_mv_end'])#params_list[1]
        discard_nc_begin = int(params_dict['discard_nc_begin'])#params_list[2]
        discard_nc_end = int(params_dict['discard_nc_end'])#params_list[3]
        window_size = int(params_dict['window_size'])#params_list[4]
        # window_not_overlap_size = window_size - int(params_dict['window_overlap_size'])#params_list[5]
        
        trial_size = int(self.config.configuration['trial_size'])
        num_trials = len(raw_X) / trial_size 

        mv_trials_begin = np.array(range(num_trials)) * self.config.configuration['trial_size'] + mv_start 
        mv_size = int(self.config.configuration['movement_trial_size_list'][subject_number])
...
#             print mv_trail_begin_ind
            ...
            #add new movement data sample
            mv_sample = raw_X[(int(mv_trial_begin) + discard_mv_begin):(int(mv_trial_begin) + mv_size - discard_mv_end),:]
            if window_size == -1:
                window_size = len(mv_sample)
                window_not_overlap_size = len(mv_sample)
            X_new, Y_new = self.extract_feature_each_window( mv_sample, X_new, Y_new, window_size, window_not_overlap_size, raw_Y[mv_trial_begin + 1], fe_params)
            
        return X_new, Y_new

    def extract_feature_each_window(self, sample, X_new, Y_new, window_size, window_not_overlap_size, label, fe_params):
        """    """
        assert len(sample.shape) == 2
        
        X_new_ex, num_of_added_samples = self.feature_extractor( sample, window_size, window_not_overlap_size, fe_params)
