...
class Main:
    ...
    def __init__(self, dir, dataset_name, learner_name,feature_extractor_name, optimization_type="", BO_selection_type="", channels = 'ALL', number_of_CSPs = -1):
        """"""
        self.dir = dir
        self.dataset_name = dataset_name
        self.config = Configuration_BCI.Configuration_BCI(dir, self.dataset_name)
                ...
                for discard_index_nc, discard_nc in enumerate(discard_ncs_begin):
                    for win_ind, window_size in enumerate(window_sizes): 
                        ...
                        params_dict = {'discard_mv_begin' : discard_mv, 'discard_mv_end' : discard_mvs_end[discard_index_mv], 
                                       ...
                                       'fe_params': fe_params, 'channel_type':self.config.channel_type + str(self.number_of_CSPs)}
    ...
    def submit_train_learner(self, subject, params_dict, optimal = False):
        ...
        if os.name == 'posix':
            ...
            if type(params_dict['fe_params']) is list:
                self.bash_script[self.python_run_cmd_ind] = self.bash_script[self.python_run_cmd_ind] + " " + self.dir + " " + self.learner_name + " " + self.feature_extractor_name + " "+ self.dataset_name + " " + subject + " " + str(self.optimization_type) + " " + self.BO_selection_type +" " + str(optimal) +  " " + ' '.join(['%s=%s' % (key, value) for (key, value) in params_dict.items() if key != 'fe_params']) + ' fe_params=' + ','.join(str(e) for e in params_dict['fe_params'])+ '\n'
            else:
                self.bash_script[self.python_run_cmd_ind] = self.bash_script[self.python_run_cmd_ind] + " " + self.dir + " " + self.learner_name + " " + self.feature_extractor_name + " "+ self.dataset_name + " " + subject +  " " + str(self.optimization_type) + " " + self.BO_selection_type + " "  + str(optimal) +  " " + ' '.join(['%s=%s' % (key, value) for (key, value) in params_dict.items()]) + '\n'
            ...
            job_runner = SJR.Single_Job_Runner(self.dir, self.learner_name, self.feature_extractor_name,self.dataset_name, subject, self.optimization_type, self.BO_selection_type, params_dict.copy(),optimal)
            job_runner.run_learner()
        
    def set_results_path(self):
        """set the results paths for both the cross validation and optimal learner"""
        
        self.results_path, self.opt_results_path = SJR.Single_Job_Runner.set_results_path(self.results_path, self.opt_results_path, self.learner_name, self.feature_extractor_name, self.optimization_type, self.BO_selection_type)
